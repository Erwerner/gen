...
public class runCrossover {
	private static final int cInitialPopulationMultiplikator = 2;
	private static final int cPopulation = 1024 * 5 * cInitialPopulationMultiplikator;
	...
	private static void checkFitness(ArrayList<Idvm> pFittestIdvm) {
		...
		System.out.println("Average steps: " + lStepCount / pFittestIdvm.size()
				+ " * " + pFittestIdvm.size());
		System.out.println("Average partner: " + 100 * lPartnerCount
				/ pFittestIdvm.size() + "/100 * " + pFittestIdvm.size());
	...
	private static void printBlockStats(ArrayList<Idvm> pFittestIdvm, int pCount) {
		...
		BlockType[] lCellBlocks = { BlockType.DEFENCE, BlockType.MOVE,
				BlockType.LIFE, BlockType.SENSOR, BlockType.NULL };
	...
	private static ArrayList<Idvm> evaluateFitness(ArrayList<Idvm> pPopulation) {
		...
		while (lFittestIdvm.size() > cPopulation
				/ cInitialPopulationMultiplikator)
	...
	private static ArrayList<Idvm> runPopulation(ArrayList<Idvm> pPopulation)
		...
		for (Idvm iIdvm : pPopulation) {
			...
			if (new Helpers().isFlagTrue("monitor"))
				new ModelMonitorIdvm().runGenome((Genome) pPopulation
						.get(Helpers.rndInt(pPopulation.size() - 1))
						.getGenomeOrigin().clone());
		}
